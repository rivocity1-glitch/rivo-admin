...
export function Supports() {
  ...
  async function fetchTickets() {
    try {
      ...
      const errors: { source: UserType; error: unknown }[] = [];
      if (customer.error) errors.push({ source: "customer", error: customer.error });
      if (vendor.error) errors.push({ source: "vendor", error: vendor.error });
      if (rider.error) errors.push({ source: "rider", error: rider.error });
  ...
  useEffect(() => {
    ...
    const channels = [
      supabase
        ...
        .on("postgres_changes", { event: "*", schema: "public", table: "customer_support_tickets" }, () => fetchTickets())
        .subscribe(),
      supabase
        .channel("admin-vendor-support-tickets")
        .on("postgres_changes", { event: "*", schema: "public", table: "vendor_support_tickets" }, () => fetchTickets())
        .subscribe(),
      supabase
        .channel("admin-rider-support-tickets")
        .on("postgres_changes", { event: "*", schema: "public", table: "rider_support_tickets" }, () => fetchTickets())
        .subscribe(),
    ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white border border-[#E2E8F0] rounded-xl p-4 flex items-center justify-between shadow-sm"><div><p className="text-xs font-semibold text-[#64748B] uppercase tracking-wider">Open Queue</p><h3 className="text-2xl font-bold text-[#0F172A] mt-1">{counters.openCount}</h3></div><div className="w-10 h-10 bg-amber-50 border border-amber-100 text-amber-500 rounded-lg flex items-center justify-center"><HelpCircle className="w-5 h-5" /></div></div>
        <div className="bg-white border border-[#E2E8F0] rounded-xl p-4 flex items-center justify-between shadow-sm"><div><p className="text-xs font-semibold text-[#64748B] uppercase tracking-wider">Unread</p><h3 className="text-2xl font-bold text-[#0F172A] mt-1">{counters.unreadCount}</h3></div><div className="w-10 h-10 bg-blue-50 border border-blue-100 text-blue-500 rounded-lg flex items-center justify-center"><MessageSquare className="w-5 h-5" /></div></div>
        <div className="bg-white border border-[#E2E8F0] rounded-xl p-4 flex items-center justify-between shadow-sm"><div><p className="text-xs font-semibold text-[#64748B] uppercase tracking-wider">High Priority</p><h3 className="text-2xl font-bold text-red-600 mt-1">{counters.highPriorityCount}</h3></div><div className="w-10 h-10 bg-red-50 border border-red-100 text-red-500 rounded-lg flex items-center justify-center"><AlertCircle className="w-5 h-5" /></div></div>
        <div className="bg-white border border-[#E2E8F0] rounded-xl p-4 flex items-center justify-between shadow-sm"><div><p className="text-xs font-semibold text-[#64748B] uppercase tracking-wider">Total Tickets</p><h3 className="text-2xl font-bold text-[#0F172A] mt-1">{tickets.length}</h3></div><div className="w-10 h-10 bg-slate-50 border border-slate-100 text-slate-500 rounded-lg flex items-center justify-center"><Clock className="w-5 h-5" /></div></div>
      </div>
      <div className="bg-white border border-[#E2E8F0] rounded-xl p-4 space-y-4 shadow-sm">
        <div className="flex flex-col md:flex-row gap-3 items-center justify-between">
          <div className="relative w-full md:w-96"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[#94A3B8]" /><input value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} placeholder="Search ticket, account UUID, subject..." className="w-full h-9 pl-9 pr-3 bg-white border border-[#E2E8F0] rounded-lg text-sm text-[#0F172A] focus:outline-none focus:border-[#22C55E]" /></div>
          <div className="flex flex-wrap items-center gap-3 w-full md:w-auto justify-end">
            <select value={typeFilter} onChange={(e) => { setTypeFilter(e.target.value); setCurrentPage(1); }} className="h-8 px-2 bg-white border border-[#E2E8F0] rounded-lg text-xs font-medium text-[#334155]"><option value="all">All Channels</option><option value="customer">Customer</option><option value="vendor">Vendor</option><option value="rider">Rider</option></select>
            <select value={priorityFilter} onChange={(e) => { setPriorityFilter(e.target.value); setCurrentPage(1); }} className="h-8 px-2 bg-white border border-[#E2E8F0] rounded-lg text-xs font-medium text-[#334155]"><option value="all">All Priorities</option><option value="high">High</option><option value="medium">Medium</option><option value="low">Low</option></select>
        ...
        <div className="flex items-center gap-2 border-t border-[#F1F5F9] pt-3 overflow-x-auto"><span className="text-xs font-semibold text-[#64748B] whitespace-nowrap">Status:</span><div className="flex border border-[#E2E8F0] rounded-lg p-0.5 bg-slate-50 shrink-0">{["all","open","in_progress","resolved","closed"].map((status) => (<button key={status} onClick={() => { setStatusFilter(status); setCurrentPage(1); }} className={cn("h-7 px-3 rounded-md text-xs font-medium capitalize", statusFilter === status ? "bg-[#22C55E] text-white shadow-sm" : "text-[#64748B] hover:text-[#0F172A]")}>{status === "in_progress" ? "In Progress" : status}</button>))}</div></div>
      </div>
      <div className="bg-white border border-[#E2E8F0] rounded-xl overflow-hidden shadow-sm">
        <div className="overflow-x-auto">
          <table className="w-full min-w-[900px]">
            <thead><tr className="border-b border-[#E2E8F0] bg-[#F8FAFC]"><th className="text-left px-4 py-3 text-xs font-medium text-[#64748B] uppercase">Ticket</th><th className="text-left px-4 py-3 text-xs font-medium text-[#64748B] uppercase">Origin</th><th className="text-left px-4 py-3 text-xs font-medium text-[#64748B] uppercase">Account UUID</th><th className="text-left px-4 py-3 text-xs font-medium text-[#64748B] uppercase">Subject</th><th className="text-left px-4 py-3 text-xs font-medium text-[#64748B] uppercase">Priority</th><th className="text-left px-4 py-3 text-xs font-medium text-[#64748B] uppercase">Status</th><th className="text-left px-4 py-3 text-xs font-medium text-[#64748B] uppercase">Last Activity</th><th className="px-4 py-3" /></tr></thead>
            <tbody className="divide-y divide-[#F1F5F9]">{isLoading ? <tr><td colSpan={8} className="text-center py-16 text-xs text-[#94A3B8]">Loading support inbox...</td></tr> : paginated.length === 0 ? <tr><td colSpan={8} className="text-center py-16 text-xs text-[#94A3B8]">No support tickets matched.</td></tr> : paginated.map((ticket) => { const userType = userTypeConfig[ticket.user_type]; const UserIcon = userType.icon; const priority = priorityConfig[ticket.priority] || priorityConfig.medium; const status = statusConfig[ticket.status] || statusConfig.open; return <tr key={`${ticket.user_type}-${ticket.id}`} onClick={() => loadConversation(ticket)} className="hover:bg-[#FAFAFA] text-sm text-[#334155] cursor-pointer transition-colors"><td className="px-4 py-3.5 font-mono font-medium text-[#0F172A]">TKT-{ticket.id.slice(0,8)}</td><td className="px-4 py-3.5"><div className={cn("inline-flex items-center gap-1.5 px-2 py-0.5 border text-xs font-bold rounded-md", userType.bg, userType.color)}><UserIcon className="w-3 h-3" />{userType.label}</div></td><td className="px-4 py-3.5 font-mono text-xs text-[#64748B] max-w-[150px] truncate">{ticket.user_id}</td><td className="px-4 py-3.5 max-w-xs"><div className="flex flex-col"><span className="font-semibold text-[#0F172A] truncate">{ticket.subject}</span>{ticket.issue_type && <span className="text-[10px] text-[#94A3B8] font-bold uppercase mt-0.5">{ticket.issue_type}</span>}{ticket.unread_for_admin && <span className="text-[10px] text-blue-600 font-bold mt-0.5">UNREAD</span>}</div></td><td className="px-4 py-3.5"><Badge variant={priority.variant} label={priority.label} className={priority.customClasses} /></td><td className="px-4 py-3.5"><Badge variant={status.variant} label={status.label} dot /></td><td className="px-4 py-3.5 text-xs text-[#64748B] whitespace-nowrap">{formatTicketDate(ticket.last_message_at || ticket.raw_date)}</td><td className="px-4 py-3.5 text-right"><ChevronRight className="w-4 h-4 text-[#94A3B8] inline" /></td></tr>; })}</tbody>
          </table>
        </div>
      ...
      {selectedTicket && <Modal open={!!selectedTicket} onClose={() => { if (!isSubmitting) setSelectedTicket(null); }} title={`Ticket #TKT-${selectedTicket.id.slice(0, 8)}`} description={`${userTypeConfig[selectedTicket.user_type].label} support conversation`} size="xl">
        <div className="space-y-5">
          ...
          <div className="bg-[#F8FAFC] border border-[#E2E8F0] rounded-xl p-4"><div className="flex items-center justify-between gap-3"><div><p className="text-[10px] text-[#64748B] font-bold uppercase tracking-wider">Original ticket</p><h4 className="text-sm font-bold text-[#0F172A] mt-1">{selectedTicket.subject}</h4></div><span className="text-[10px] text-[#94A3B8]">{formatTicketDate(selectedTicket.raw_date)}</span></div><p className="text-xs text-[#475569] leading-relaxed bg-white border border-[#F1F5F9] p-3 rounded-lg font-medium whitespace-pre-wrap mt-3">{selectedTicket.message}</p></div>
          {selectedTicket.screenshot_url && <div className="border border-[#E2E8F0] rounded-xl overflow-hidden bg-slate-50"><button type="button" onClick={() => setPreviewScreenshot(!previewScreenshot)} className="w-full flex items-center justify-between px-3 py-2 text-xs font-bold text-[#334155] bg-white border-b border-[#E2E8F0]"><span className="flex items-center gap-1.5"><FileText className="w-3.5 h-3.5 text-[#10B981]" />Attachment</span><span className="flex items-center gap-1 text-[#22C55E]"><Eye className="w-3.5 h-3.5" />{previewScreenshot ? "Collapse" : "Preview"}</span></button>{previewScreenshot && <div className="p-3 bg-slate-900 flex justify-center max-h-72 overflow-hidden"><img src={selectedTicket.screenshot_url} alt="Support attachment" className="max-w-full max-h-64 object-contain rounded" /></div>}</div>}
          <div className="border border-[#E2E8F0] rounded-xl overflow-hidden"><div className="px-4 py-3 bg-white border-b border-[#E2E8F0] flex items-center justify-between"><div><h4 className="text-sm font-bold text-[#0F172A] flex items-center gap-2"><MessageSquare className="w-4 h-4 text-[#22C55E]" />Conversation</h4><p className="text-[10px] text-[#94A3B8] mt-0.5">All user and Admin replies are stored here.</p></div><span className="text-[10px] font-bold text-[#64748B]">{messages.length} messages</span></div><div className="p-4 bg-[#F8FAFC] max-h-[360px] overflow-y-auto space-y-3">{isMessagesLoading ? <div className="py-12 text-center text-xs text-[#94A3B8]">Loading conversation...</div> : messages.length === 0 ? <div className="py-10 text-center text-xs text-[#94A3B8]">No replies yet. The original ticket is shown above.</div> : messages.map((message) => { const isAdmin = message.sender_type === "admin"; return <div key={message.id} className={cn("flex", isAdmin ? "justify-end" : "justify-start")}><div className={cn("max-w-[82%] rounded-2xl px-4 py-3 border shadow-sm", isAdmin ? "bg-[#E8FBF0] border-[#BBF7D0]" : "bg-white border-[#E2E8F0]")}><div className="flex items-center justify-between gap-5 mb-1.5"><span className={cn("text-[10px] font-black uppercase", isAdmin ? "text-[#15803D]" : userTypeConfig[selectedTicket.user_type].color)}>{isAdmin ? "Rivo Support" : userTypeConfig[selectedTicket.user_type].label}</span><span className="text-[9px] text-[#94A3B8]">{formatMessageDate(message.created_at)}</span></div><p className="text-xs text-[#334155] leading-relaxed whitespace-pre-wrap">{message.message}</p>{message.attachment_url && <a href={message.attachment_url} target="_blank" rel="noreferrer" className="block text-[10px] text-[#16A34A] font-semibold mt-2">View attachment</a>}</div></div>; })}</div><div className="p-4 bg-white border-t border-[#E2E8F0]"><div className="flex gap-2 items-end"><textarea value={replyText} onChange={(e) => setReplyText(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); handleReply(); } }} disabled={isSubmitting || selectedTicket.status === "closed"} placeholder={selectedTicket.status === "closed" ? "Closed tickets cannot receive replies." : "Write a reply to the user..."} rows={3} className="flex-1 resize-none rounded-xl border border-[#E2E8F0] bg-[#F8FAFC] p-3 text-xs text-[#0F172A] focus:outline-none focus:border-[#22C55E] disabled:opacity-60" /><Button variant="primary" className="h-10 px-4 text-xs flex items-center gap-1.5" onClick={handleReply} disabled={isSubmitting || !replyText.trim() || selectedTicket.status === "closed"}><Send className="w-3.5 h-3.5" />{isSubmitting ? "Sending..." : "Send Reply"}</Button></div><p className="text-[9px] text-[#94A3B8] mt-1.5">Enter to send · Shift+Enter for a new line</p></div></div>
          ...
          <div className="pt-3 border-t border-[#F1F5F9] flex flex-wrap gap-2">{!showDeleteConfirm && <Button variant="outline" className="h-9 px-3 border-red-200 text-red-600 hover:bg-red-50 flex items-center gap-1" onClick={() => setShowDeleteConfirm(true)} disabled={isSubmitting}><Trash2 className="w-3.5 h-3.5" />Delete Ticket</Button>}{selectedTicket.status === "open" && <Button variant="primary" className="flex-1 text-xs h-9" onClick={() => handleTicketMutation("in_progress")} disabled={isSubmitting || showDeleteConfirm}>Claim & Start Progress</Button>}{(selectedTicket.status === "open" || selectedTicket.status === "in_progress") && <Button variant="primary" className="flex-1 text-xs h-9 bg-[#22C55E] hover:bg-[#16A34A] border-0 flex items-center justify-center gap-1" onClick={() => handleTicketMutation("resolved")} disabled={isSubmitting || showDeleteConfirm}><CheckCircle className="w-3.5 h-3.5" />Mark Resolved</Button>}{selectedTicket.status === "resolved" && <Button variant="secondary" className="flex-1 text-xs h-9 flex items-center justify-center gap-1" onClick={() => handleTicketMutation("closed")} disabled={isSubmitting || showDeleteConfirm}><XCircle className="w-3.5 h-3.5" />Archive & Close</Button>}{(selectedTicket.status === "resolved" || selectedTicket.status === "closed") && <Button variant="outline" className="h-9 text-xs" onClick={() => handleTicketMutation("open")} disabled={isSubmitting || showDeleteConfirm}>Reopen</Button>}<Button variant="outline" className="h-9 text-xs" onClick={() => setSelectedTicket(null)} disabled={isSubmitting}>Close Inspection</Button></div>
        </div>
      </Modal>}
    </div>
  );
