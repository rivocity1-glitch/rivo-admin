...
export function Customers() {
  ...
  async function fetchCustomers() {
    try {
      ...
      const {
        ...
        .select(
          "customer_id, address_line1, city, state, pin_code"
        )
        .eq("is_default", true);

      if (addressesError) {
        console.error(
          "Failed fetching default customer addresses:",
          addressesError
        );
      }
      ...
      (addressesData || []).forEach((addr) => {
        if (addr.customer_id) {
          ...
          addressMap[addr.customer_id] =
            pieces.join(", ") || "No address provided";
        }
      });
      ...
      if (ordersError) {
        console.error(
          "Failed fetching orders for aggregation:",
          ordersError
        );
      }

      const ordersAggregation: Record<
        string,
        {
          count: number;
          spent: number;
      ...
      (ordersData || []).forEach((order) => {
        ...
        if (!ordersAggregation[order.customer_id]) {
          ordersAggregation[order.customer_id] = {
            count: 0,
            spent: 0,
            lastOrderTime: null,
          };
        ...
        ordersAggregation[order.customer_id].spent += Number(
      ...
      if (viewCustomer) {
        const updatedTarget = mapped.find(
          (c) => c.id === viewCustomer.id
        );

        if (updatedTarget) {
          setViewCustomer(updatedTarget);
        }
      }
    } catch (err) {
      console.error(
        "Failed parsing live customers database:",
        err
      );
    } finally {
      setIsLoading(false);
  ...
  async function fetchCustomerModalDetails(
    customerId: string
  ) {
    try {
      const {
      ...
      if (addressIds.length > 0) {
        const {
          ...
          .from("customer_addresses")
          .select(
            "id, address_line1, address_line2, landmark, city, state, pin_code"
          )
          .in("id", addressIds);
      ...
      const {
        ...
        .from("refunds")
        .select(
          "id, order_id, customer_id, vendor_id, amount, reason, status, created_at"
        )
        .eq("customer_id", customerId)
        .order("created_at", {
          ascending: false,
      ...
      const {
        ...
        .from("customer_addresses")
        .select(
          "address_line1, address_line2, landmark, city, state, pin_code"
        )
        .eq("customer_id", customerId)
        .eq("is_default", true)
        .maybeSingle();
      ...
      console.error(
        "Failed fetching context profiles metrics:",
        err
      );
    }
  }
  ...
  async function handleAddCustomer() {
    ...
    try {
      ...
      if (err.code === "23505") {
        alert(
          "This email address is already registered to an existing customer."
        );
      } else {
        alert(
  ...
  async function toggleBlock(
    ...
    try {
      ...
      alert(
        `Customer status updated to ${nextStatus}.`
      );
    } catch (err: any) {
      console.error(
        "Failed status update toggling:",
        err
      );
   ...
   *
   * IMPORTANT:
   * The Admin PWA does NOT directly delete:
   *
   * - customers
   * - customer_addresses
   * - orders
   ...
   * - auth.users
   *
   * The delete-user Edge Function is the only deletion path.
   *
   * This prevents foreign-key failures and ensures that the
   * Supabase Auth account is deleted together with the profile.
   */
  ...
  async function handleDeleteCustomer(
    ...
    const confirmation = window.confirm(
      `Are you sure you want to permanently delete customer account "${name}"?\n\nThe deletion will be processed by the Rivo super-admin deletion service. Historical records that must remain for order integrity will be preserved where required.\n\nThis action cannot be undone.`
    );
    ...
    try {
      ...
      if (error) {
        ...
        throw new Error(
          error.message ||
            "The customer deletion service could not be reached."
        );
      }

      if (!data) {
        throw new Error(
          "The deletion service returned no response."
        );
      }
      ...
      if (!data.success) {
        throw new Error(
          data.error ||
            "Customer deletion was rejected."
        );
      }
  ...
  return (
    <div>
      <PageHeader
        title="Customers"
        description={`${customerList.filter(
          (c) =>
            c.status === "active"
        ).length} active customer profiles verified`}
        actions={
          <Button
            variant="primary"
            size="sm"
        ...
        <div className="flex items-center gap-1 border border-[#E2E8F0] rounded-lg p-1 bg-white">
          {[
            ...
            <button
              ...
              className={cn(
                "h-7 px-3 rounded-md text-xs font-medium capitalize",
                statusFilter === s
          ...
          <tbody className="divide-y divide-[#F1F5F9]">
            {isLoading ? (
              <tr>
                <td
                  colSpan={7}
                  className="text-center py-16 text-sm text-[#94A3B8]"
                >
                  Loading data platform metrics...
                </td>
              </tr>
            ) : paginated.length === 0 ? (
              <tr>
                <td
                  ...
                  className="text-center py-16 text-sm text-[#94A3B8]"
                >
                  No user customer records found.
                </td>
              </tr>
            ) : (
              paginated.map(
                        ...
                        <div>
                          <button
                            onClick={() => {
                              ...
                              fetchCustomerModalDetails(
                                customer.id
                              );
                            }}
                            className="text-sm font-medium text-[#0F172A] hover:text-[#22C55E] text-left"
                          >
                            {customer.name}
                          </button>
                      ...
                      <Dropdown
                        ...
                        trigger={
                          <button
                            ...
                            className="h-7 w-7 flex items-center justify-center rounded-md text-[#64748B] hover:bg-[#F1F5F9] disabled:opacity-50"
                          >
      ...
      >
        <div className="space-y-4">
          ...
          <Input
            ...
            placeholder="Flat No, Street, Landmark, City"
            value={formAddress}
            onChange={(e) =>
              setFormAddress(
                e.target.value
          ...
          {activeTab === "profile" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  },
                  {
                    label:
                      "Average Order Value",
                    value: `₹${profileMetrics.avgOrderValue.toLocaleString(
                      undefined,
                      {
              ...
              <div className="bg-[#F8FAFC] border border-[#E2E8F0] rounded-xl p-3.5">
                ...
                <p className="text-sm text-[#334155] font-medium pl-5">
                  {
                    profileMetrics.fullAddress
                  }
                </p>
              </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div className="bg-[#EFF6FF] border border-[#DBEAFE] rounded-lg p-4 text-center">
                  <p className="text-2xl font-semibold text-blue-700">
                    ₹
                    {profileMetrics.totalSpent.toLocaleString()}
                  </p>
              ...
              <div className="grid grid-cols-3 gap-2 text-center text-xs text-[#64748B] font-medium bg-[#F8FAFC] p-3 rounded-xl border border-[#E2E8F0]">
                <div>
                  ...
                  <span className="font-bold text-[#16A34A]">
                    {
                      profileMetrics.completedOrders
                    }
                  </span>
                </div>
                ...
                <div>
                  ...
                  <span className="font-bold text-rose-600">
                    {
                      profileMetrics.cancelledOrders
                    }
                  </span>
                </div>
              </div>
                ...
                customerOrders.map(
                  (order) => {
                    ...
                    return (
                      <div
                        key={order.id}
                        className="border border-[#E2E8F0] rounded-xl p-3 bg-white space-y-2 text-xs"
                      >
                          ...
                          <div className="mt-2 p-3 bg-[#F8FAFC] border border-[#E2E8F0] rounded-xl space-y-2 text-[#475569] animate-in fade-in duration-100">
                            ...
                            <div className="space-y-1 divide-y divide-[#E2E8F0]/50">
                              ...
                              {(
                                ...
                                (
                                  ...
                                  <div
                                    key={idx}
                                    className="grid grid-cols-4 py-1 items-center"
                                  >
                            ...
                            <div className="pt-2 border-t border-[#E2E8F0] space-y-1">
                              ...
                              <div className="break-words">
                                ...
                                <span className="font-medium text-[#0F172A]">
                                  {
                                    order.delivery_address_computed
                                  }
                                </span>
                              </div>
                            </div>
              ...
              {customerRefunds.length === 0 ? (
                ...
                customerRefunds.map(
                  (refund) => (
                    <div
                      key={refund.id}
                      className="border border-[#E2E8F0] rounded-xl p-3 bg-white space-y-1.5 text-xs"
                    >
